    ...
    <package-base @start-upload="openFileSelector()">
        ...
        <template #actions>
            <div class="package-actions__inner" v-if="hasUploads && !uploading">
                ...
                <button class="package-actions__button widget-button widget-button--primary" :disabled="!canConfirmUploads || removingUploads" @click="confirmUploads">
                    {{ $t('ui.packages.uploadApply') }}
                </button>
            ...
            <div class="package-actions__inner" v-else-if="totalChanges && !uploading">
                ...
                <button-group
                    class="package-actions__button-group"
                    align-top
                    type="primary"
                    icon="update"
                ...
                <button class="package-actions__button widget-button widget-button--alert" :disabled="!canResetChanges && !confirmed.length" @click="resetChanges">
                    {{ $t('ui.packages.changesReset') }}
                </button>
            </div>
        </template>
    </package-base>
...
export default {
    ...
    computed: {
        ...
        addedPackages: (vm) =>
            vm.visibleRequired
                .concat(vm.visibleAdded)
                .filter((pkg) => !vm.packageMissing(pkg.name))
                .sort(sortPackages),
        installedPackages: (vm) =>
            vm.visibleRequired
                .filter((pkg) => vm.packageMissing(pkg.name))
                .concat(vm.visibleInstalled)
                .sort(sortPackages),
